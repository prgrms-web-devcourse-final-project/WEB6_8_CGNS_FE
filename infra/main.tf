...
resource "aws_vpc" "vpc_1_team11" {
  ...
  tags = {
    Name = "${var.prefix}-vpc-1_team11"
  }
}

resource "aws_subnet" "subnet_1" {
  vpc_id                  = aws_vpc.vpc_1_team11.id
...
docker run -d \
  --name postgresql_1 \
  --restart unless-stopped \
  -v /dockerProjects/postgresql_1/volumes/var/lib/postgresql/data:/var/lib/postgresql/data \
  --network common \
  -p 5432:5432 \
...
until docker exec postgresql_1 pg_isready -U postgres &> /dev/null; do
