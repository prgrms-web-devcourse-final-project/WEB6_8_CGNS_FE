...
import com.back.koreaTravelGuide.domain.ai.weather.dto.CombinedWeatherData
import com.back.koreaTravelGuide.domain.ai.weather.dto.DayWeatherInfo
import com.back.koreaTravelGuide.domain.ai.weather.dto.MidTermMetricsDay
import com.back.koreaTravelGuide.domain.ai.weather.dto.MidTermMetricsRegion
...
import com.back.koreaTravelGuide.domain.ai.weather.dto.WeatherDetails
...
class WeatherService(
    ...
    fun getMidTermNarrative(
        ...
        val candidateCodes =
            deriveCandidateRegionCodes(
                forecastSummary = response.forecast,
                fallbackRegionCode = response.regionCode,
            )

        return MidTermNarrativeResult(
            baseTime = response.baseTime,
            primaryRegion = response.region,
            summary = response.forecast,
            candidateRegionCodes = candidateCodes,
        )
    }

    fun getMidTermMetrics(
        regionCodes: List<String>,
        baseTime: String?,
        days: List<Int>?,
    ): MidTermMetricsResult {
        require(regionCodes.isNotEmpty()) { "regionCodes는 최소 1개 이상 제공해야 합니다." }
        ...
        val regionSummaries =
            regionCodes
                ...
                .map { requestedCode ->
                    ...
                    val daySummaries =
                        normalizedDays.mapNotNull { day ->
                            val dayInfo = result.details.getDay(day)
                            dayInfo?.let {
                                MidTermMetricsDay(
                                    dayOffset = day,
                                    date = it.date,
                                    minTemp = it.temperature?.minTemp,
                                    maxTemp = it.temperature?.maxTemp,
                                    amRainPercent = it.precipitation?.amRainPercent,
                                    pmRainPercent = it.precipitation?.pmRainPercent,
                                    amWeather = it.precipitation?.amWeather,
                                    pmWeather = it.precipitation?.pmWeather,
                                )
                            }
                        }

                    MidTermMetricsRegion(
    ...
    private fun deriveCandidateRegionCodes(
        forecastSummary: String,
        fallbackRegionCode: String,
    ): List<String> {
        val normalizedForecast = forecastSummary.replace("\n", " ")
        val candidates = mutableSetOf<String>()

        REGION_NAME_TO_CODE_FOR_CANDIDATE.forEach { (regionName, code) ->
            val index = normalizedForecast.indexOf(regionName)
            if (index >= 0) {
                val windowStart = (index - FAVORABLE_WINDOW_RANGE).coerceAtLeast(0)
                val windowEnd = (index + regionName.length + FAVORABLE_WINDOW_RANGE).coerceAtMost(normalizedForecast.length)
                val windowText = normalizedForecast.substring(windowStart, windowEnd)

                val isFavorable = FAVORABLE_KEYWORDS.any { keyword -> windowText.contains(keyword) }
                if (isFavorable) {
                    candidates += code
                }
            }
    ...
    companion object {
        private const val FAVORABLE_WINDOW_RANGE = 20

        private val FAVORABLE_KEYWORDS = listOf("맑", "화창", "쾌청", "쾌적", "좋은", "맑겠")
        ...
        private val REGION_NAME_TO_CODE_FOR_CANDIDATE =
            REGION_MAP +
                mapOf(
                    "경기도" to "11B20601",
                    "강원도" to "11D10301",
